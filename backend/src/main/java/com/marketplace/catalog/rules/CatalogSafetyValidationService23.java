...
@Slf4j
@Service
public class CatalogSafetyValidationService23 {

    public CatalogComplianceRule23 validateProduct(Product product) {
        boolean clean = product.getTitle() != null && !product.getTitle().toLowerCase().contains("prohibited");
        return CatalogComplianceRule23.builder()
                .product(product)
                .ruleType("SAFETY_COMPLIANCE_STANDARDS_23")
