...
@Slf4j
@Service
public class CatalogSafetyValidationService26 {

    public CatalogComplianceRule26 validateProduct(Product product) {
        boolean clean = product.getTitle() != null && !product.getTitle().toLowerCase().contains("prohibited");
        return CatalogComplianceRule26.builder()
                .product(product)
                .ruleType("SAFETY_COMPLIANCE_STANDARDS_26")
