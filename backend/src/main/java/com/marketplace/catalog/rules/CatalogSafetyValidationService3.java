...
@Slf4j
@Service
public class CatalogSafetyValidationService3 {

    public CatalogComplianceRule3 validateProduct(Product product) {
        boolean clean = product.getTitle() != null && !product.getTitle().toLowerCase().contains("prohibited");
        return CatalogComplianceRule3.builder()
                .product(product)
                .ruleType("SAFETY_COMPLIANCE_STANDARDS_3")
