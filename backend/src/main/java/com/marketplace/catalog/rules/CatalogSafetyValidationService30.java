...
@Slf4j
@Service
public class CatalogSafetyValidationService30 {

    public CatalogComplianceRule30 validateProduct(Product product) {
        boolean clean = product.getTitle() != null && !product.getTitle().toLowerCase().contains("prohibited");
        return CatalogComplianceRule30.builder()
                .product(product)
                .ruleType("SAFETY_COMPLIANCE_STANDARDS_30")
