...
@Slf4j
@Service
public class CatalogSafetyValidationService33 {

    public CatalogComplianceRule33 validateProduct(Product product) {
        boolean clean = product.getTitle() != null && !product.getTitle().toLowerCase().contains("prohibited");
        return CatalogComplianceRule33.builder()
                .product(product)
                .ruleType("SAFETY_COMPLIANCE_STANDARDS_33")
