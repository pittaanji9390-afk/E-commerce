...
@Slf4j
@Service
public class CatalogSafetyValidationService44 {

    public CatalogComplianceRule44 validateProduct(Product product) {
        boolean clean = product.getTitle() != null && !product.getTitle().toLowerCase().contains("prohibited");
        return CatalogComplianceRule44.builder()
                .product(product)
                .ruleType("SAFETY_COMPLIANCE_STANDARDS_44")
