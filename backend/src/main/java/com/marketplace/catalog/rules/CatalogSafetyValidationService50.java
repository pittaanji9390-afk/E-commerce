...
@Slf4j
@Service
public class CatalogSafetyValidationService50 {

    public CatalogComplianceRule50 validateProduct(Product product) {
        boolean clean = product.getTitle() != null && !product.getTitle().toLowerCase().contains("prohibited");
        return CatalogComplianceRule50.builder()
                .product(product)
                .ruleType("SAFETY_COMPLIANCE_STANDARDS_50")
