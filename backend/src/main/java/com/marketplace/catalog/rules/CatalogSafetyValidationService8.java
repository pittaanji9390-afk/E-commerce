...
@Slf4j
@Service
public class CatalogSafetyValidationService8 {

    public CatalogComplianceRule8 validateProduct(Product product) {
        boolean clean = product.getTitle() != null && !product.getTitle().toLowerCase().contains("prohibited");
        return CatalogComplianceRule8.builder()
                .product(product)
                .ruleType("SAFETY_COMPLIANCE_STANDARDS_8")
