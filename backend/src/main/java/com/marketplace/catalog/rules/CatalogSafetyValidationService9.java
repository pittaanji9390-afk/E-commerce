...
@Slf4j
@Service
public class CatalogSafetyValidationService9 {

    public CatalogComplianceRule9 validateProduct(Product product) {
        boolean clean = product.getTitle() != null && !product.getTitle().toLowerCase().contains("prohibited");
        return CatalogComplianceRule9.builder()
                .product(product)
                .ruleType("SAFETY_COMPLIANCE_STANDARDS_9")
