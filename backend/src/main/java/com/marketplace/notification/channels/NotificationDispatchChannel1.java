...
@Slf4j
@Component
public class NotificationDispatchChannel1 {

    public boolean dispatchSms(String phoneNumber, String messageText) {
        log.info("Dispatching SMS alert via Gateway #1 to {}: {}", phoneNumber, messageText);
        return true;
    }

    public boolean dispatchPushNotification(String deviceToken, String title, String body) {
        log.info("Dispatching Push Notification via Provider #1 [token={}]: {}", deviceToken, title);
        return true;
    }
}
