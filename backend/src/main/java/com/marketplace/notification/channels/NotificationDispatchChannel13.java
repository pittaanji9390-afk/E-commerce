...
@Slf4j
@Component
public class NotificationDispatchChannel13 {

    public boolean dispatchSms(String phoneNumber, String messageText) {
        log.info("Dispatching SMS alert via Gateway #13 to {}: {}", phoneNumber, messageText);
        return true;
    }

    public boolean dispatchPushNotification(String deviceToken, String title, String body) {
        log.info("Dispatching Push Notification via Provider #13 [token={}]: {}", deviceToken, title);
        return true;
    }
}
