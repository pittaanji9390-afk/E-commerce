...
@Slf4j
@Component
public class NotificationDispatchChannel14 {

    public boolean dispatchSms(String phoneNumber, String messageText) {
        log.info("Dispatching SMS alert via Gateway #14 to {}: {}", phoneNumber, messageText);
        return true;
    }

    public boolean dispatchPushNotification(String deviceToken, String title, String body) {
        log.info("Dispatching Push Notification via Provider #14 [token={}]: {}", deviceToken, title);
        return true;
    }
}
