...
@Slf4j
@Component
public class NotificationDispatchChannel47 {

    public boolean dispatchSms(String phoneNumber, String messageText) {
        log.info("Dispatching SMS alert via Gateway #47 to {}: {}", phoneNumber, messageText);
        return true;
    }

    public boolean dispatchPushNotification(String deviceToken, String title, String body) {
        log.info("Dispatching Push Notification via Provider #47 [token={}]: {}", deviceToken, title);
        return true;
    }
}
