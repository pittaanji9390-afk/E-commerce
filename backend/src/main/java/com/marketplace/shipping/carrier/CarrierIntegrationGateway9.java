...
@Slf4j
@Component
public class CarrierIntegrationGateway9 {

    public String createShippingLabel(Shipment shipment, String serviceLevel) {
        String trackingNumber = "TRK-9-" + System.currentTimeMillis() + "-" + UUID.randomUUID().toString().substring(0, 6).toUpperCase();
        log.info("Dispatch label generated via carrier provider #9 [tracking={}]", trackingNumber);
    ...
    public BigDecimal fetchLiveQuote(String originZip, String destZip, BigDecimal weightKg) {
        BigDecimal base = BigDecimal.valueOf(8.50 + (9 % 5));
